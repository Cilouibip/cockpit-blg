...
const STREAM='commerce_declared_snapshot',OVERVIEW='notion_commerce_overview',DAY='notion_commerce_day',MEMBERS='notion_commerce_membership';
...
async function rowsForRun(db:Database,runId:string,metric:string,max=1000){const rows=await db.select('source_aggregates',{eq:{sync_run_id:runId,metric_key:metric},order:'period_from,dimensions_key',limit:max});if(rows.length>=max)throw Error('COMMERCE_REPORT_READ_LIMIT');return rows;}
...
async function storedPublicationMatches(db:Database,run:Row,report:CommerceReport,memberKeys:string[],fingerprint:string){
 ...
 if(!info||info.reportFingerprint!==fingerprint||info.version!==NOTION_COMMERCE_VERSION||info.observedAt!==report.observedAt||!validCounts(info.totals)||digest(info.totals)!==digest(report.totals)||info.dailyCount!==report.daily.length||info.dailyHash!==digest(report.daily)||info.membershipHash!==report.membershipHash||info.sourceMemberCount!==memberKeys.length||info.sourceMemberHash!==digest(memberKeys))return false;
...
export async function publishNotionCommerceReport(db:Database,config:NotionCommerceConfig,report:CommerceReport){
 ...
 const fingerprint=digest({profile,version:report.version,observedAt:report.observedAt,startedAt:report.startedAt,totals:report.totals,coverage:report.coverage,daily:report.daily,membershipHash:report.membershipHash,memberKeys});
...
export interface CommerceDashboard {available:boolean;source:string;definitionState:'pending_business_choice';observedAt:string|null;counts:CommerceCounters|null;coverage:CommerceReport['coverage']|null;reason:string;runId?:string;provisional?:boolean}
...
export async function readNotionCommerceReport(db:Database,filters:DashboardFilters,env:Record<string,string|undefined>=process.env):Promise<CommerceDashboard|null>{
 ...
 const unavailable=(reason:string):CommerceDashboard=>({available:false,source:'Notion · achats déclarés et paiements rapprochés',definitionState:'pending_business_choice',observedAt:null,counts:null,coverage:null,reason});
 ...
 for(const run of runs){
  ...
  return {available:true,source:'Notion · achats déclarés et paiements rapprochés',definitionState:'pending_business_choice',observedAt,counts,coverage:info.coverage as CommerceReport['coverage'],runId:String(run.id),provisional:filters.to>=observedDay,reason:'Sous-totaux des Parcours Notion disponibles. Personnes accompagnées, classement payeur, paiement rattaché et dates divergentes restent distincts ; historique métier non exhaustif.'};
