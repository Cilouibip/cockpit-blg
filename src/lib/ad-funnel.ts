...
export interface AdFunnelRow {
 key:string;kind:'ad'|'organic'|'unattributed';label:string;campaignLabel:string|null;
 ...
 leadsBooked:number;
	appointmentsBooked:number;appointmentsAttended:number;appointmentsNoShow:number;appointmentsCancelled:number;appointmentsRescheduled:number;appointmentsUpcoming:number;appointmentsUnknown:number;
...
export interface AdFunnelReport {
 ...
 coverage:{
  ...
  appointments:{rows:number;linkedPeople:number;attendanceFromBusiness:number};
...
function originFrom(record:Record<string,unknown>|null,basis:'first_touch'|'arrival'):AdFunnelOrigin{
 ...
 const adId=numericId(record.ad),campaignId=numericId(record.campaign),adsetId=numericId(record.adset),link=text(record.linkId,36),linkId=link&&UUID.test(link)?link.toLowerCase():null,source=text(record.source,60);
 if(adId||campaignId||adsetId||linkId||source)return {adId,campaignId,adsetId,linkId,source,basis};
...
export function originKeyFor(o:{adId?:string|null;linkId?:string|null;campaignId?:string|null;source?:string|null}):string {
 return o.adId?'ad:'+o.adId:o.linkId?'link:'+o.linkId:o.campaignId?'campaign:'+o.campaignId:o.source?ORGANIC:UNATTRIBUTED;
}
...
function firstTouchInstant(raw:unknown,occurredAt:string):string {
 ...
 if(at<EARLIEST_MEASURABLE||at>occurredAt)return occurredAt;
 return at;
}
type Candidate={origin:AdFunnelOrigin;at:string;observationId:string};
const earlier=(a:Candidate,b:Candidate)=>a.at<b.at||(a.at===b.at&&(a.origin.basis==='first_touch'&&b.origin.basis!=='first_touch'||(a.origin.basis===b.origin.basis&&a.observationId<b.observationId)));
...
function emptyRow(key:string,kind:AdFunnelRow['kind'],label:string):AdFunnelRow{
	 return {key,kind,label,campaignLabel:null,adId:null,campaignId:null,adsetId:null,creativeId:null,linkIds:[],links:[],tunnels:[],spendMinor:null,impressions:null,outboundClicks:null,visitors:emptyCounts(),pageviews:emptyCounts(),visitorsWithFirstOrigin:null,optin:{quiz:null,masterclass:null,all:null,unlinkableVisitors:null,registrationsWithoutVisitor:0,registrationsOutsideCohort:0},registrations:0,registrationsByTunnel:{quiz:0,masterclass:0},uniqueRegistrants:0,uniqueLeads:0,uniqueLeadsByTunnel:{quiz:0,masterclass:0},knownBefore:0,unresolvedIdentity:0,leadsBooked:0,appointmentsBooked:0,appointmentsAttended:0,appointmentsNoShow:0,appointmentsCancelled:0,appointmentsRescheduled:0,appointmentsUpcoming:0,appointmentsUnknown:0,firstSalesConfirmed:null,firstSalesReconciled:null,firstSalesPending:null,cashMinor:null,refundsMinor:null,attributionBasis:{firstTouch:0,arrival:0},rates:{optin:rate(null,null,RATE_REASONS.cohort),booking:rate(0,0,RATE_REASONS.booking),attendance:rate(0,0,RATE_REASONS.attendance)}};
}
/** Présence d'un créneau : classification courante Notion du prospect (prospects.business) quand elle date ce créneau, sinon le statut de la ligne rendez-vous. */
export function appointmentOutcome(appointment:Row,business:Row|null,day:string|null):'attended'|'no_show'|'cancelled'|'rescheduled'|'scheduled'|'unknown' {
	 const status=String(appointment.status??'unknown');
	 if(status==='cancelled'||status==='rescheduled')return status;
 const b=business&&typeof business==='object'?business:null;
 const scheduledDay=b&&typeof b.scheduledDay==='string'?b.scheduledDay:null,attendance=b&&typeof b.attendance==='string'?b.attendance:null;
 if(day&&scheduledDay===day&&attendance&&attendance!=='unknown')return attendance==='show_up'?'attended':attendance==='no_show'?'no_show':attendance==='cancelled'?'cancelled':'scheduled';
	 return status==='attended'||status==='no_show'||status==='scheduled'?status:'unknown';
}
...
function matchesFilters(row:AdFunnelRow,source:SourceFilter,campaign:string):boolean {
 if(source==='paid'&&row.kind!=='ad')return false;
 if(source==='organic'&&row.kind!=='organic')return false;
 if(source==='unknown'&&row.kind!=='unattributed')return false;
...
export async function buildAdFunnel(db:Database,filters:AdFunnelFilters,options:AdFunnelOptions={}):Promise<AdFunnelReport>{
 ...
 const siteId=env.WIX_SITE_ID??null;
 ...
 const firstRequest=new Map<string,Observation>(),best=new Map<string,Candidate>();
 for(const o of requests){
  ...
  const arrival=originFrom(o.origin,'arrival'),first=originFrom(o.firstTouch,'first_touch');
  ...
  for(const c of candidates){const prior=best.get(o.personId);if(!prior||earlier(c,prior))best.set(o.personId,c);}
 }
 const personOrigin=new Map<string,AdFunnelOrigin>();for(const [person,c] of best)personOrigin.set(person,c.origin);
 ...
 const prospectIds=[...new Set(appointments.map(a=>a.prospect_id?String(a.prospect_id):'').filter(Boolean))];
 const prospectById=new Map<string,{personId:string|null;business:Row|null}>();
 for(let i=0;i<prospectIds.length;i+=150){const chunk=prospectIds.slice(i,i+150);for(const p of await db.select('prospects',{in:{id:chunk},columns:['id','person_id','business'],limit:1000}))prospectById.set(String(p.id),{personId:p.person_id?String(p.person_id):null,business:p.business&&typeof p.business==='object'?p.business as Row:null});}
 // 5. Ventes payées déjà classées (relevé Notion-commerce), reliées à la personne par le Client Notion.
	 const commerce=await readNotionCommerceReport(db,{from:'1900-01-01',to:'2999-12-31',source:'all',tunnel:'all',campaign:'all',compare:false},env,commerceReadMemo());
 ...
 for(const o of requests){
  ...
  addLink(row,own.linkId);if(origin.linkId)addLink(row,origin.linkId);
  ...
  const registered=registeredPersonsByRow.get(key)??new Set<string>();registered.add(o.personId!);registeredPersonsByRow.set(key,registered);
	  if(!classifiedPersons.has(o.personId!)){
	   ...
	   if(first?.id===o.id&&!(clientDay&&clientDay<o.occurredDay!)){row.uniqueLeads++;row.uniqueLeadsByTunnel[tunnel]++;}else row.knownBefore++;
	  }
 }
 ...
 for(const a of appointments){
  ...
  if(day!>=today)row.appointmentsUpcoming++;
 ...
 const matchedRegistrations=new Set<string>();
	 const knownBeforeInstant=(person:string,instant:string,day:string)=>{const first=firstRequest.get(person),client=earliestClientDay.get(person);return !!(first?.occurredAt&&first.occurredAt<instant)||!!(client&&client<day);};
 if(cohort?.available){
  const tunnelsWanted:FunnelTunnel[]=filters.tunnel==='all'?['quiz','masterclass']:[filters.tunnel];
 ...
 const withRates=(r:AdFunnelRow|AdFunnelTotals)=>{
  ...
  r.rates={optin:!cohort?.available?rate(null,null,cohort?.reason??RATE_REASONS.cohort):counts?rate(counts.registered,counts.visitors,RATE_REASONS.optin):rate(0,0,RATE_REASONS.optin),booking:rate(r.leadsBooked,r.uniqueRegistrants,RATE_REASONS.booking),attendance:rate(r.appointmentsAttended,r.appointmentsAttended+r.appointmentsNoShow,RATE_REASONS.attendance)};
 };
 ...
 for(const r of ordered){
  ...
  totals.registrations+=r.registrations;totals.registrationsByTunnel.quiz+=r.registrationsByTunnel.quiz;totals.registrationsByTunnel.masterclass+=r.registrationsByTunnel.masterclass;totals.uniqueRegistrants+=r.uniqueRegistrants;totals.uniqueLeads+=r.uniqueLeads;totals.uniqueLeadsByTunnel.quiz+=r.uniqueLeadsByTunnel.quiz;totals.uniqueLeadsByTunnel.masterclass+=r.uniqueLeadsByTunnel.masterclass;totals.knownBefore+=r.knownBefore;totals.unresolvedIdentity+=r.unresolvedIdentity;totals.leadsBooked+=r.leadsBooked;
	  totals.appointmentsBooked+=r.appointmentsBooked;totals.appointmentsAttended+=r.appointmentsAttended;totals.appointmentsNoShow+=r.appointmentsNoShow;totals.appointmentsCancelled+=r.appointmentsCancelled;totals.appointmentsRescheduled+=r.appointmentsRescheduled;totals.appointmentsUpcoming+=r.appointmentsUpcoming;totals.appointmentsUnknown+=r.appointmentsUnknown;
 ...
 return {
  ...
  definitions:{
   ...
   uniqueLeads:'Personnes dont la première inscription tombe dans la période, sans antériorité Client. Une personne déjà connue ne redevient pas un lead.',
