...
export function applyDashboardRollup(response:DashboardResponse,r:DashboardRollup,filters:DashboardFilters,mode:DataMode){
 ...
 const adDays=r.meta.daily.map(d=>d.date).sort();
 const adCoverage=mode==='demo'?observation:`Montants connus des annonces${adDays.length?` du ${adDays[0]} au ${adDays.at(-1)}`:''} ; couverture temporelle des annonces non établie. Comparaison désactivée.`;
 const adCompleteness=mode==='demo'?'complete' as const:'partial' as const;
 set('spend',spend,{updatedAt:r.meta.observedAt,completeness:adCompleteness,...(!adReady&&campaign.startsWith('meta-creative:')?{unavailableReason:'Les identifiants de créatives ne sont pas encore raccordés aux annonces importées.'}:{}),coverage:adReady?adCoverage:'Aucune partition Meta publiée et compatible.'});
 ...
 set('impressions',impressions,{updatedAt:r.meta.observedAt,coverage:adCoverage,completeness:adCompleteness});
 set('clicks',clicks,{updatedAt:r.meta.observedAt,coverage:adCoverage,completeness:adCompleteness});
 const ratio=(id:string,n:number|null,d:number|null,scale:number)=>set(id,mode==='demo'&&n!==null&&d!==null&&d>0?n/d*scale:null,{numerator:n,denominator:d,updatedAt:r.meta.observedAt,coverage:adCoverage,completeness:adCompleteness,...(mode==='live'?{unavailableReason:'La couverture des annonces ne permet pas de calculer un ratio fiable sur cette sélection.'}:{})});
