...
export interface CommerceClient {id:string;emailKey:string|null;emailBisKey:string|null;startedDay:string|null;prospectIds:string[];binomeIds:string[]}
export interface CommercePayment {id:string;clientIds:string[];emailKey:string|null;providerId:string|null;day:string|null;rawDate:string|null;amountMinor:number|null;status:string}
...
export interface CommerceSnapshot {clients:CommerceClient[];payments:CommercePayment[];parcours:CommerceParcours[];startedAt:string;observedAt:string;paginationComplete:boolean;sourceCounts:Record<string,number>}
...
export interface CommerceReport {version:string;startedAt:string;observedAt:string;daily:{date:string;counts:CommerceCounters}[];totals:CommerceCounters;members:CommerceMember[];accompanimentMembers:CommerceAccompanimentMember[];sourceMembers:{clients:string[];payments:string[];parcours:string[]};membershipHash:string;coverage:{sourceRows:Record<string,number>;publishedPopulation:string;historicalExhaustivity:false;undatedParcours:number;undatedClientStarts:number;futureClientStarts:number;unresolvedClientRelations:number;deduplicatedPayments:number;firstPaymentObservedDay:string|null};paginationComplete:true}
...
export function buildNotionCommerceReport(snapshot:CommerceSnapshot):CommerceReport {
 ...
 for(const list of [snapshot.clients,snapshot.payments,snapshot.parcours])if(new Set(list.map(r=>r.id)).size!==list.length)throw Error('DUPLICATE_SOURCE_ROWS');
 const clients=new Map(snapshot.clients.map(r=>[r.id,r])),canonicalPayments:CommercePayment[]=[],provider=new Map<string,CommercePayment>();let deduplicatedPayments=0;
 ...
 const sourceMembers={clients:snapshot.clients.map(c=>c.id).sort(),payments:snapshot.payments.map(p=>p.id).sort(),parcours:snapshot.parcours.map(p=>p.id).sort()};
