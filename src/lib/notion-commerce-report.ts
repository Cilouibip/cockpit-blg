...
export interface CommerceReport {version:string;startedAt:string;observedAt:string;daily:{date:string;counts:CommerceCounters}[];totals:CommerceCounters;paidSales:PaidSalesReport;members:CommerceMember[];accompanimentMembers:CommerceAccompanimentMember[];sourceMembers:{clients:string[];payments:string[];schedules:string[];parcours:string[]};membershipHash:string;coverage:{retainedArchivedClients?:number;sourceRows:Record<string,number>;publishedPopulation:string;historicalExhaustivity:false;undatedParcours:number;undatedClientStarts:number;futureClientStarts:number;unresolvedClientRelations:number;deduplicatedPayments:number;firstPaymentObservedDay:string|null};paginationComplete:true}
