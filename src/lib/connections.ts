...
export async function connections():Promise<ConnectionsResponse> {
  ...
  const connection=(id:string,name:string,configured:boolean,limits:string[],syncable=false):Connection=>{
    ...
    return {id,name,status:demo?'demo':!configured?'missing':last?.status==='failed'?'error':complete?'connected':'partial',
      summary:demo?'Données synthétiques de test.':!configured?'Connexion serveur à renseigner.':!available?'Accès fourni ; base du cockpit à installer.':!last?'Accès fourni ; aucun import effectué.':complete?'Dernier import enregistré.':last.status==='empty'?'Lecture réussie, aucune ligne reçue.':last.status==='partial'||last.status==='running'?'Lecture en cours ou partielle ; reprise enregistrée.':'Dernier import en échec.',
  ...
  return {mode:config.mode,connections:[
    ...
    connection('notion','Notion · commercial',!!process.env.NOTION_TOKEN&&!!process.env.NOTION_DATA_SOURCE_ID,['Lecture des propriétés commerciales autorisées uniquement.','Dates métier historiques conservées ; créations seules signalées à part. Présences selon classification courante Notion, sans reconstituer les créneaux remplacés.'],true),
    connection('wix','Wix · encaissements',!!process.env.WIX_API_KEY,['CA issu de la synthèse des paiements Wix, actualisé pour la période choisie.','Périmètre Wix seulement ; rapprochement historique Notion et paiements par client encore incomplet.'],true),
    connection('posthog','PostHog · parcours',!!process.env.POSTHOG_PERSONAL_API_KEY,['Étapes et visiteurs mesurés, actualisés avec la période du cockpit.','Données agrégées ; les inscriptions serveur et les liens avec les ventes restent distincts.']),
