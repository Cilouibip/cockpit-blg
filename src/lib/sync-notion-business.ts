...
interface Claim {busy:boolean;runId:string;lease?:string;checkpoint?:{intervals:Interval[];version?:number;mode?:'full'|'delta';page?:number;inventoryThrough?:string;partitions?:{from:string;to:string}[]};from?:string;to?:string;rowsRead?:number}
...
export async function synchronizeNotionChunk(options:{db?:Database;env?:Record<string,string|undefined>;reader?:(config:NotionConfig)=>ReturnType<typeof syncNotion>;maxPages?:number;schemaReader?:typeof readNotionBusinessSchema;inventoryReader?:typeof readNotionInventoryPage;fetcher?:typeof fetch}={}) {
 ...
 let read=0,pages=0,pageNumber=claim.checkpoint?.page??0;const partitions=[...(claim.checkpoint?.partitions??[])];
 try {
  ...
  if(!intervals.length){
   ...
   return {status:result.status,counts:{read:(claim.rowsRead??0)+read,accepted:result.count,rejected:0,pages,changed:result.changed},coverage:{complete:result.status!=='failed',mode:result.mode??claim.checkpoint?.mode??'full',inventoryThrough:result.inventoryThrough,from:claim.from,to:claim.to,reason:result.reason??'Modifications Notion publiées ; inventaire complet et dates métier suivis séparément.'},runId:claim.runId};
  }
