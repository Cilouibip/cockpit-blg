...
export async function readLiveKpiFunnel(db: Database, filters: DashboardFilters, options: { env?: NodeJS.ProcessEnv; now?: string; includeTests?: boolean } = {}): Promise<KpiFunnelResponse> {
 ...
 const [meta, posthog, email, leadRows, prospects, runs, ads, links, appointments, commerce] = await Promise.all([
  ...
  pagedRows(db,'link_revisions',{order:'id',columns:['id','campaign','medium']}),
 ...
 const daily: KpiFunnelDay[] = dates.map(date=> {
  const row: KpiFunnelDay={date,partial_day:date>=Temporal.Instant.from(now).toZonedDateTimeISO('Europe/Paris').toPlainDate().toString()?'Journée en cours ou à venir':null,...missing()};
  const metaDay=meta.days.get(date);
  if(metaDay&&metaSupported){const rows=metaDay.rows.filter(r=>metaCampaigns.includes(String(r.data.campaignId)));for(const key of ['spend_eur','impressions','link_clicks','unique_link_clicks_campaign_sum','landing_page_views','booking_meta_attributed'] as const)row[key]=rows.length?sum(rows.map(r=>number(r.data[key]))):null;}
  if(formsAvailable&&formsAt&&Date.parse(startOfParisDay(date))<Date.parse(formsAt))row.wix_form_submission_occurrences=requests.filter(r=>r.occurred_day===date).length;
  const ph=posthog.days.get(date);
  if(ph){const rows=ph.rows.filter(r=>!isExcludedTestTraffic({includeTests},r.data)&&matches(observationOrigin({origin:{...r.data,linkId:r.data.link},firstTouch:null})));row.booking_clicks=rows.filter(r=>r.data.kind==='click').reduce((n,r)=>n+Number(r.data.sessions),0);row.booking_confirmed_browser=rows.filter(r=>r.data.kind==='confirmed').reduce((n,r)=>n+Number(r.data.sessions),0);}
  if(cohortComplete&&notionAt&&formsAt&&[notionAt,formsAt].every(at=>Date.parse(startOfParisDay(date))<Date.parse(at))){
  ...
  if(moneyAvailable&&commerceAt&&clientsAt&&[commerceAt,clientsAt,formsAt!,notionAt!].every(at=>Date.parse(startOfParisDay(date))<Date.parse(at))){
 ...
 const emailRows=dates.flatMap(day=>email.days.get(day)?.rows??[]),emailCovered=dates.every(day=>email.days.has(day));
 ...
 const cover=(field_group:string,through:string|null,available:boolean,reason:string,attempt?:Row|null)=>coverage.push({field_group,status:available?'available':'missing',...(through?{through,stale:Date.parse(now)-Date.parse(through)>3600000}:{}),reason,...(attempt?{last_attempt:String(attempt.started_at),...(attempt.status==='failed'?{last_error:String(attempt.error_code??'SYNC_UNIT_FAILED')}:{})}:{})});
 cover('Diffusion Meta',freshThrough(meta),metaSupported&&daily.every(r=>r.spend_eur!==null),metaSupported?'Campagnes masterclass identifiées ; absence de ligne différente de zéro.':'Ce filtre ne permet pas de répartir la dépense campagne. Les mesures Meta restent non mesurées.',meta.latestAttempt);
 cover('Occurrences du formulaire Wix',formsAt,formsAvailable,'Formulaire de la masterclass actuelle ; occurrences et contacts distincts restent séparés.');
 cover('Clics bilan et confirmations navigateur',freshThrough(posthog),dates.every(d=>posthog.days.has(d)),'Sessions, première origine et essais explicites ; ce ne sont pas des rendez-vous réalisés.',posthog.latestAttempt);
 cover('Rendez-vous et présence',notionAt,cohortComplete&&!!notionRun,`${people.size} contacts raccordés pour ${identities.size} contacts distincts de la période.`);
 ...
 cover('Ventes payées et cash',commerceAt,moneyAvailable,commerceAttempt?.status==='failed'?'Dernier rapport valide conservé ; une tentative commerciale a échoué. Les cas à rapprocher restent non mesurés.':'Paiements de la cohorte raccordée, ventes confirmées distinctes des cas à rapprocher.',commerceAttempt);
 cover('Activité email',freshThrough(email),emailCovered,'Séquence commune à trois formulaires. Le rapprochement porte sur les destinataires ; il ne prouve pas le formulaire déclencheur.',email.latestAttempt);
 for(const [group,reason] of [['CTA oral','Aucun signal validé au passage du CTA oral.'],['Offres faites','Aucun champ source validé.'],['CA contracté','Aucune source contractuelle alignée.']])cover(group,null,false,reason);
 for(const failure of failures)cover(`Lecture ${failure}`,null,false,'La lecture a échoué ; aucune absence convertie en zéro.');
 ...
 for(const {rows} of posthog.days.values())for(const row of rows){
  ...
  const item=breakdown.get(key)??{label:origin.adId?String(adById.get(origin.adId)?.ad_name??`Publicité ${origin.adId}`):origin.linkId?'Lien identifié':origin.campaignId?`Campagne ${origin.campaignId}`:origin.source??'Non attribué',ad_id:origin.adId,booking_click_sessions:0,booking_confirmed_browser:0};
