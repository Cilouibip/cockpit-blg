...
export async function synchronize(source:'meta'|'notion',from?:string,to?:string){
  ...
  if(source==='notion')return synchronizeNotionChunk();
  const today=Temporal.Now.plainDateISO('Europe/Paris');
  return syncMetaAccountPeriod(from??today.subtract({days:35}).toString(),to??today.add({days:1}).toString());
}
export async function synchronizeMetaAds(from?:string,to?:string){
  if(getConfig().mode==='demo')throw new AppError('Données de démonstration.',409,'demo_mode');
  ...
  const namespace=(process.env.META_AD_ACCOUNT_ID||'').replace(/^act_/,'');
  ...
  const run=await db.rpc<string>('begin_sync_stream',{p_source:source,p_namespace:namespace,p_from:start,p_to:end,p_profile:`${process.env.META_API_VERSION||'v23.0'}-ad-day-none`,p_stream:'ad_daily',p_coverage_kind:'aggregate_period',p_date_from:fromDay,p_date_to:toDay});
  ...
  const result=await syncMeta({accessToken:process.env.META_ACCESS_TOKEN,accountId:namespace,apiVersion:process.env.META_API_VERSION||'v23.0',from:fromDay,to:toDay,maxPages:20,commitPage:commit});
