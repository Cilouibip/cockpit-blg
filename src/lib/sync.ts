...
import { syncNotionSnapshot, BLG_NOTION_FIELDS } from '../connectors/notion';
...
export async function synchronize(source:'meta'|'notion',from?:string,to?:string){
  ...
  const result=source==='meta'?await syncMeta({accessToken:process.env.META_ACCESS_TOKEN,accountId:namespace,apiVersion:process.env.META_API_VERSION||'v23.0',from:fromDay,to:toDay,maxPages:20,commitPage:commit}):await syncNotionSnapshot({token:process.env.NOTION_TOKEN,dataSourceId:namespace,fields:BLG_NOTION_FIELDS,mappingVersion:'blg-commercial-2026-09-07',from:start,to:end,maxPages:100,commitPage:commit});
