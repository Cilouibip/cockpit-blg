...
/** Lecture des inscriptions Wix : formulaires (masterclass) puis quiz, reprise au point enregistré tant que la lecture est partielle. Une famille en échec arrête et se voit. */
async function refreshLeadEntries(request:(path:string,init:{method:string;body:string;timeoutMs:number})=>Promise<import('../lib/refresh-plan').RefreshResult>,progress:(message:string)=>void):Promise<import('../lib/refresh-plan').RefreshResult>{
  for(const family of ['forms','quiz'] as const){
    const label=family==='forms'?'masterclass':'quiz';
    const result=await refreshNotionToCompletion(()=>request(`/api/sync/lead-entries?family=${family}`,{method:'POST',body:'{}',timeoutMs:75_000}),read=>progress(`Inscriptions ${label} : ${read} lignes lues, lecture en cours…`));
    if(result.status!=='complete'&&result.status!=='empty')return {...result,coverage:{reason:result.coverage?.reason??`Inscriptions ${label} : lecture non terminée.`}};
  }
...
function Connections({ data, refresh, announce }: { data: ConnectionsResponse; refresh: () => void; announce: (message: string) => void }) {
  ...
  return <><p className="blg-note"><Icon name="lock" />Les sources sont consultées en lecture seule. Un accès disponible ne signifie pas que l’alimentation automatique est installée.</p>{error && <p className="blg-inline-error" role="alert">{error}</p>}<div className="blg-connections">{data.connections.map((connection, index) => <section className="blg-panel blg-connection" key={connection.id}><div className="blg-connection-heading"><span className="blg-number">{String(index + 1).padStart(2, '0')}</span><div><h2>{connection.name}</h2><span className="blg-connection-status">{connection.status === 'connected' ? '✓ ' : connection.status === 'error' ? '! ' : '— '}{connection.id==='database'&&connection.status==='connected'?'Accès disponible':status[connection.status]}</span></div></div><p>{connection.summary}</p><dl><div><dt>Dernière lecture complète</dt><dd>{formatDate(connection.lastSyncAt, true)}</dd></div>{connection.dataAsOf && <div><dt>Données lues jusqu’au</dt><dd>{formatDate(connection.dataAsOf, true)}</dd></div>}{connection.lastAttemptAt && <div><dt>Dernière tentative</dt><dd>{formatDate(connection.lastAttemptAt, true)}</dd></div>}<div><dt>Couverture</dt><dd>{connection.coverage}</dd></div></dl>{connection.limits.length > 0 && <div className="blg-connection-limits"><span className="blg-eyebrow">LIMITES ACTUELLES</span><ul>{connection.limits.map(limit => <li key={limit}>{limit}</li>)}</ul></div>}{connection.canSync && ['meta', 'notion', 'wix_inscriptions', 'notion_commerce'].includes(connection.id) && <button className="blg-button" disabled={!!busy} onClick={() => sync(connection)}><Icon name="refresh" size={16} />{busy === connection.id ? 'Lecture en cours…' : 'Lire les données disponibles'}</button>}</section>)}</div><p className="blg-footnote">Aucune donnée source n’est modifiée depuis ce cockpit. L’installation des pages et le déploiement sont suivis séparément.</p></>;
}
...
export default function Cockpit({ mode, user }: { mode: DataMode; user: string }) {
  ...
  const [journeyFilters, setJourneyFilters] = useState<DashboardFilters>(() => ({ ...defaultFilters(), compare:false }));
  const [journeyAds, setJourneyAds] = useState<{id:string;label:string}[]>([]);
  const [journeyTunnel, setJourneyTunnel] = useState<'masterclass'|'quiz'>('masterclass');
  ...
  const dashboard = useRemote<DashboardResponse>(view === 'results' ? `/api/dashboard?${filtersQuery(filters)}` : null, revision);
  const reports = usePostHogReports(filters, mode==='live' && view==='results' && !!dashboard.data && !dashboard.loading && dashboard.data.period.from===filters.from && dashboard.data.period.to===filters.to, revision, dashboard.setData,filters.tunnel==='masterclass'?'masterclass':'quiz');
  ...
  async function refreshSources() {
    if(view==='journey'){refresh();return;}
    if(view==='sales'){
