...
const copy: Record<string, { label: string; description: string; missing?: string; formula?: string }> = {
  cash: { label: 'CA encaissé', description: 'L’argent reçu sur la période, remboursements déduits. Montant TTC, avant frais.' },
  spend: { label: 'Dépenses publicitaires', description: 'Le montant dépensé sur Meta pour les annonces sélectionnées.' },
  leads: { label: 'Leads uniques', description: 'Les personnes inscrites au quiz ou à la masterclass. Une personne présente dans les deux est comptée une seule fois.' },
  appointments: { label: 'RDV réalisés', description: 'Les rendez-vous auxquels le prospect était présent, selon la date prévue du rendez-vous.' },
  new_clients: { label: 'Nouveaux clients', description: 'Les personnes devenues clientes pour la première fois.', missing: 'L’historique des achats manque pour distinguer les nouveaux clients des clients existants.' },
  roas: { label: 'ROAS attribué', description: 'Le chiffre d’affaires attribué à la publicité pour chaque euro dépensé.', missing: 'Les achats ne sont pas encore reliés aux publicités.', formula: 'CA attribué, net des remboursements ÷ dépenses de la même cohorte. Attribution au dernier contact non direct dans les 30 jours ; revenus suivis sur 90 jours après le contact.' },
  ad_customer_cost: { label: 'Coût pub. par nouveau client', description: 'La dépense publicitaire moyenne pour acquérir un nouveau client.', missing: 'Les nouveaux clients ne sont pas encore reliés aux dépenses.', formula: 'Dépenses publicitaires ÷ nouveaux clients de la même cohorte. Les autres coûts marketing et commerciaux sont exclus.' },
  contracted: { label: 'CA contracté', description: 'Le montant TTC des engagements signés sur la période. Une partie peut rester à encaisser.' },
  arrivals: { label: 'Visites des pages', description: 'Les sessions mesurées sur les pages du quiz et de la masterclass. Plusieurs appareils peuvent compter comme plusieurs visites.' },
  impressions: { label: 'Affichages des publicités', description: 'Le nombre de fois où les publicités sélectionnées ont été affichées sur Meta.' },
  clicks: { label: 'Clics vers le site', description: 'Les clics qui quittent Meta pour ouvrir la page de destination.' },
  ctr: { label: 'Taux de clic vers le site', description: 'La part des affichages publicitaires qui donnent lieu à un clic vers le site.', formula: 'Clics vers le site ÷ affichages des publicités × 100.' },
  cpc: { label: 'Coût par clic vers le site', description: 'Le coût moyen d’un clic vers le site.', formula: 'Dépenses publicitaires ÷ clics vers le site.' },
  cpm: { label: 'Coût pour 1 000 affichages', description: 'Le coût moyen de mille affichages publicitaires.', formula: 'Dépenses publicitaires ÷ affichages × 1 000.' },
  cpl: { label: 'Coût par lead acquis', description: 'La dépense publicitaire moyenne par première inscription attribuée.', missing: 'Les premières inscriptions ne sont pas encore reliées aux publicités.', formula: 'Dépenses de la cohorte ÷ premières inscriptions attribuées à cette cohorte.' },
  showup: { label: 'Taux de présence aux rendez-vous', description: 'La part des rendez-vous passés où le prospect était présent.', formula: 'RDV réalisés ÷ (RDV réalisés + absences) × 100. Les annulations, reports et présences non renseignées sont exclus.' },
  closing: { label: 'Taux de conversion après rendez-vous', description: 'La part des personnes reçues qui signent ensuite un engagement, dans la même fenêtre de suivi.', missing: 'Les engagements signés ne sont pas encore reliés aux rendez-vous.' },
  cac: { label: 'Coût total par client', description: 'Le coût d’acquisition complet : publicité, marketing et vente.', missing: 'Les coûts marketing et commerciaux hors publicité manquent.', formula: 'Tous les coûts d’acquisition retenus ÷ clients acquis sur le même périmètre.' },
};
const name = (metric: Metric) => copy[metric.id]?.label ?? metric.label;
...
function Change({ metric, compare }: { metric: Metric; compare: boolean }) {
  if (!compare || metric.value === null || metric.previous == null) return null;
...
function Card({ metric, compare, onOpen }: { metric: Metric; compare: boolean; onOpen: (metric: Metric) => void }) {
  return <article className="a-d-card results-kpi" data-metric={metric.id}><h3>{name(metric)}</h3><div className="a-d-value">{formatNumber(metric.value, metric.unit)}</div><div className="results-card-foot"><span>{metric.value === null ? <span className="results-state">Indisponible</span> : metric.id === 'contracted' ? <><span className="results-state">Partiel</span><Change metric={metric} compare={compare} /></> : <Change metric={metric} compare={compare} />}</span><button className="a-button a-n-quiet" onClick={() => onOpen(metric)} aria-label={`Voir le détail : ${name(metric)}`}>Détail <Arrow /></button></div></article>;
}

function MetricDrawer({ metric, onClose, compare }: { metric: Metric; onClose: () => void; compare: boolean }) {
  const text = copy[metric.id];
  // Keep the supplied evidence intact; these substitutions only make its vocabulary readable.
  const coverage = metric.coverage.replaceAll('Périmètre synthétique de test.', '').replaceAll('issues inconnues', 'présences non renseignées').replaceAll('Les emplacements Notion courants sont séparés.', 'Les dates indiquées dans Notion ne suffisent pas à compter une présence.');
  const source = ({ 'Occurrences commerciales prouvées': 'Suivi des rendez-vous', 'Inscriptions serveur vérifiées': 'Inscriptions au quiz et à la masterclass', 'Premier encaissement prouvé': 'Historique des paiements', 'Cohorte d’acquisition · dernier contact non direct': 'Publicités et paiements attribués' } as Record<string, string>)[metric.source] ?? metric.source;
  return <ResultDialog title={name(metric)} onClose={onClose}><div className="a-d-value">{formatNumber(metric.value, metric.unit)}</div><Change metric={metric} compare={compare} /><p>{text?.description ?? metric.definition}</p>{metric.value === null && <p className="results-reason">{text?.missing ?? metric.unavailableReason ?? 'Les données nécessaires ne sont pas encore disponibles.'}</p>}
    {metric.id === 'contracted' && <p className="results-reason">Le total porte sur les contrats importés ; certains engagements peuvent manquer.</p>}
    <details className="results-detail-section"><summary>Détails du calcul <Arrow down /></summary><div><p>{text?.formula ?? metric.definition}</p>{metric.numerator !== undefined && <dl className="a-d-detail-facts"><div><dt>{metric.id === 'showup' ? 'RDV réalisés' : metric.id === 'ctr' ? 'Clics vers le site' : ['cpc','cpm'].includes(metric.id) ? 'Dépenses publicitaires' : 'Numérateur'}</dt><dd>{formatNumber(metric.numerator, ['cpc','cpm'].includes(metric.id) ? 'eur' : 'count')}</dd></div><div><dt>{metric.id === 'showup' ? 'RDV réalisés + absences' : ['ctr','cpm'].includes(metric.id) ? 'Affichages' : metric.id === 'cpc' ? 'Clics vers le site' : 'Dénominateur'}</dt><dd>{formatNumber(metric.denominator)}</dd></div></dl>}{coverage && <p>{coverage}</p>}{compare && <dl className="a-d-detail-facts"><div><dt>Période précédente</dt><dd>{metric.previous == null ? 'Non disponible' : formatNumber(metric.previous, metric.unit)}</dd></div></dl>}</div></details>
    <details className="results-detail-section"><summary>Source et mise à jour <Arrow down /></summary><dl className="a-d-detail-facts"><div><dt>Source</dt><dd>{source}</dd></div><div><dt>Dernière mise à jour</dt><dd>{metric.updatedAt ? formatDate(metric.updatedAt, true) : 'Aucune donnée disponible'}</dd></div></dl></details>
  </ResultDialog>;
}
