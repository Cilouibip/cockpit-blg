...
function Card({ metric, compare, onOpen, booked }: { metric: Metric; compare: boolean; onOpen: (metric: Metric) => void; booked?: Metric }) {
  ...
  if (metric.id === 'appointments') return <article className="a-d-card results-kpi" data-metric={metric.id}><h3>Rendez-vous</h3><div className="results-appointment-counts"><div><strong>{formatNumber(booked?.value ?? null)}</strong><span>Réservés</span></div><div><strong>{formatNumber(metric.value)}</strong><span>Réalisés</span></div></div><div className="results-card-foot"><span className="results-state">Sur la période choisie</span><button className="a-button a-n-quiet" onClick={() => onOpen(metric)} aria-label="Voir les rendez-vous réservés et réalisés">Détail <Arrow /></button></div></article>;
