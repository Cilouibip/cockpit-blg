...
function AdFunnelTable({ filters }: { filters: DashboardFilters }) {
  ...
  const leads = (row: Line) => <>{formatNumber(row.uniqueRegistrants)}<span className="a-t-context">{formatNumber(row.registrations)} inscription{row.registrations > 1 ? 's' : ''}{row.uniqueLeads !== row.uniqueRegistrants ? ` · ${formatNumber(row.uniqueLeads)} nouveau${row.uniqueLeads > 1 ? 'x' : ''}` : ''}{row.knownBefore ? ` · ${formatNumber(row.knownBefore)} déjà connu${row.knownBefore > 1 ? 's' : ''}` : ''}{row.unresolvedIdentity ? ` · ${formatNumber(row.unresolvedIdentity)} sans identité` : ''}</span></>;
  const booked = (row: Line) => <>{formatNumber(row.appointmentsBooked)}<span className="a-t-context">{[row.appointmentsUpcoming ? `${formatNumber(row.appointmentsUpcoming)} à venir` : null, row.appointmentsCancelled ? `${formatNumber(row.appointmentsCancelled)} annulé${row.appointmentsCancelled > 1 ? 's' : ''}` : null, row.appointmentsRescheduled ? `${formatNumber(row.appointmentsRescheduled)} reporté${row.appointmentsRescheduled > 1 ? 's' : ''}` : null].filter(Boolean).join(' · ')}</span></>;
  const attended = (row: Line) => <>{formatNumber(row.appointmentsAttended)}<span className="a-t-context">{[row.appointmentsNoShow ? `${formatNumber(row.appointmentsNoShow)} absent${row.appointmentsNoShow > 1 ? 's' : ''}` : null, row.appointmentsUnknown ? `${formatNumber(row.appointmentsUnknown)} sans issue` : null].filter(Boolean).join(' · ')}</span></>;
