...
import KpiFunnelTable from './KpiFunnelTable';
...
export function ResultsFilters({ draft, applied, onChange, onApply, campaigns, busy, error }: { draft: DashboardFilters; applied: DashboardFilters; onChange: (value: DashboardFilters) => void; onApply: (event: FormEvent) => void; campaigns: DashboardResponse['campaigns']; busy: boolean; error: string }) {
  ...
  const campaignChoices = [...new Map([
    ...CURRENT_MASTERCLASS_CAMPAIGNS.map(campaign => ({ id: `meta:${campaign.id}`, label: campaign.label })),
    ...campaigns,
  ].map(campaign => [campaign.id, campaign])).values()];
  const quickAd = /^meta-ad:(\d+)$/.exec(draft.campaign)?.[1];
  const quickCampaignId = /^meta:(\d+)$/.exec(draft.campaign)?.[1] ?? (quickAd ? currentCampaignForAd(quickAd) : null);
  const quickCampaign = CURRENT_MASTERCLASS_CAMPAIGNS.some(campaign => campaign.id === quickCampaignId) ? `meta:${quickCampaignId}` : '';
  ...
  return <form className="results-filter-form" onSubmit={submit}>
    <div className="results-toolbar"><label className="a-f-field results-period" data-emphasis="quiet"><span className="blg-sr-only">Période rapide</span><span className="a-field-shell"><select aria-label="Période rapide" value={period} onChange={e => choosePeriod(e.target.value as PeriodId)}>{periods.map(p => <option key={p.id} value={p.id}>{p.label}</option>)}<option value="custom">Dates personnalisées</option></select></span></label><div className="results-dates"><label className="a-f-field" data-emphasis="quiet"><span>Du</span><span className="a-field-shell"><input type="date" value={draft.from} onChange={e => { setPeriod('custom'); onChange({ ...draft, from: e.target.value }); }} required /></span></label><label className="a-f-field" data-emphasis="quiet"><span>Au</span><span className="a-field-shell"><input type="date" value={draft.to} onChange={e => { setPeriod('custom'); onChange({ ...draft, to: e.target.value }); }} required /></span></label></div>
      <label className="a-f-field results-period" data-emphasis="quiet"><span className="blg-sr-only">Campagne actuelle</span><span className="a-field-shell"><select aria-label="Campagne actuelle" value={quickCampaign || (draft.campaign ? '__other__' : '')} onChange={e => onChange({ ...draft, source: e.target.value ? 'paid' : draft.source, tunnel: e.target.value ? 'masterclass' : draft.tunnel, campaign: e.target.value })}><option value="">Toutes les campagnes</option>{draft.campaign && !quickCampaign && <option value="__other__">Autre sélection</option>}{CURRENT_MASTERCLASS_CAMPAIGNS.map(campaign => <option key={campaign.id} value={`meta:${campaign.id}`}>{campaign.label}</option>)}</select></span></label>
...
function Card({ metric, compare, onOpen, booked }: { metric: Metric; compare: boolean; onOpen: (metric: Metric) => void; booked?: Metric }) {
  const state = resultState(metric);
  if (metric.id === 'appointments') return <article className="a-d-card results-kpi" data-metric={metric.id}><h3>Rendez-vous</h3><div className="results-appointment-counts"><div><strong>{formatNumber(booked?.value ?? null)}</strong><span>Réservés · date de réservation</span></div><div><strong>{formatNumber(metric.value)}</strong><span>Réalisés · date du créneau</span></div></div><div className="results-card-foot"><span className="results-state">Deux bases de dates : un écart entre les deux n’est pas une erreur.</span><button className="a-button a-n-quiet" onClick={() => onOpen(metric)} aria-label="Voir les rendez-vous réservés et réalisés">Détail <Arrow /></button></div></article>;
...
export function ReportStatus({reports,retry}:{reports:PostHogClientState;retry?:()=>void}) {
  ...
  return <>{current && current.state!=='ready' && <p className="results-reason" role="status">{current.message}</p>}{previous && previous.state!=='ready' && <p className="results-reason" role="status">{previous.state==='waiting'&&previous.retryable?'La comparaison est en attente. Reprends son chargement.':['loading','waiting'].includes(previous.state)?'Chargement de la comparaison…':'La comparaison n’est pas disponible pour cette période.'}</p>}{retry && [current,previous].some(s=>s?.state==='failed'||(s?.state==='waiting'&&s.retryable)) && <button className="a-button a-secondary" onClick={retry}>Réessayer ces chiffres</button>}</>;
}
function MetricDrawer({ metric, onClose, filters,posthog={},retryPosthog }: { metric: Metric; onClose: () => void; filters: DashboardFilters;posthog?:PostHogClientState;retryPosthog?:()=>void }) {
  ...
  if (['appointments','booked'].includes(metric.id)) return <ResultDialog title="Les rendez-vous" onClose={onClose}><BookingResults filters={filters} initialView="reserved" /></ResultDialog>;
  return <ResultDialog title={name(metric)} onClose={onClose}>
    ...
    {metric.value === null && <span className="results-state">{report?.state==='waiting'&&report.retryable?'En attente':report && ['loading','waiting'].includes(report.state)?'Chargement…':'Indisponible'}</span>}
...
function AdFunnelTable({ filters }: { filters: DashboardFilters }) {
  ...
  const selectedAd = /^meta-ad:(\d+)$/.exec(filters.campaign)?.[1];
  const currentCampaign = /^meta:(\d+)$/.exec(filters.campaign)?.[1] ?? (selectedAd ? currentCampaignForAd(selectedAd) : null);
  const focused = CURRENT_MASTERCLASS_CAMPAIGNS.some(campaign => campaign.id === currentCampaign);
  const [open, setOpen] = useState(() => CURRENT_MASTERCLASS_CAMPAIGNS.some(campaign => campaign.id === currentCampaign)); const [loaded, setLoaded] = useState<{ query: string; value: AdFunnelReport } | null>(null); const [busyQuery, setBusyQuery] = useState<string | null>(null); const [failure, setFailure] = useState<{ query: string; message: string } | null>(null); const [attempt, setAttempt] = useState(0); const id = useId();
  useEffect(() => { if (CURRENT_MASTERCLASS_CAMPAIGNS.some(campaign => campaign.id === currentCampaign)) setOpen(true); }, [currentCampaign]);
  ...
  const booked = (row: Line) => <>{formatNumber(row.appointmentsReserved > 0 || report?.coverage.appointments.available ? row.appointmentsReserved : null)}</>;
  ...
  return <section className="results-accordion"><h3><button className="a-h-toggle" aria-expanded={open} aria-controls={id} onClick={() => setOpen(!open)}><span className="a-h-marker a-luminous-marker">05</span><span className="a-h-copy"><span className="a-h-title">Par publicité, du clic à l’encaissement</span></span><span className="a-h-meta">{report && <span className="a-h-count">{formatNumber(report.rows.length)} lignes</span>}<span className="a-h-chevron"><Arrow /></span></span></button></h3>
    <div id={id} hidden={!open}><div className="results-campaigns" aria-busy={busy}>
      {!focused && <><label className="blg-traffic-toggle"><input type="checkbox" checked={includeTests} onChange={event => setIncludeTests(event.target.checked)} />Inclure les essais dans ce tableau</label><p className="a-d-data-note">{includeTests ? 'Visiteurs et inscriptions : essais identifiés inclus.' : 'Visiteurs et inscriptions : essais explicitement identifiés exclus.'} Les dépenses et clics Meta gardent le périmètre publicitaire sélectionné.</p></>}
      ...
      {report && <>
        {!focused && <><p className="a-d-data-note">Chaque personne est créditée à la première publicité mesurée, même si elle s’inscrit plus tard par une autre. Visiteurs par date de visite, inscrits par date d’inscription, RDV réservés par date de réservation, RDV réalisés par date du créneau, ventes et encaissements par date de paiement. RDV réservés et RDV réalisés n’ont donc pas la même base de date : un écart entre les deux n’est pas une erreur.</p>{report.notices.map(notice => <p className="a-d-data-note" key={notice}>{notice}</p>)}</>}
        {report.rows.length ? <div className="a-t-table-wrap results-table-scroll" tabIndex={0} role="region" aria-label="Résultats par publicité"><table className="a-t-table results-table results-table-wide"><thead><tr><th>Publicité</th><th>Dépenses</th><th>Clics Meta</th><th>Visiteurs</th><th>Inscrits</th><th>Opt-in</th><th>RDV réservés</th><th>Réservation</th><th>RDV réalisés</th><th>Présence</th><th>Nouvelles ventes</th><th>Encaissé</th></tr></thead><tbody>
          {report.rows.map(row => <tr className="a-t-row" key={row.key}><td><span className="a-t-name">{row.label}</span>{!focused && <span className="a-t-context">{[row.campaignLabel, row.creativeId ? `créative ${row.creativeId}` : row.kind === 'ad' && row.adId ? 'créative non lue' : null, tunnelLabel(row), row.links.length ? `${row.links.length} lien${row.links.length > 1 ? 's' : ''} cockpit` : null].filter(Boolean).join(' · ')}</span>}</td>{cells(row)}</tr>)}
        ...
        {!focused && <><p className="a-d-data-note">Opt-in = visiteurs entrés dans la période (identifiant de navigateur) devenus inscrits ensuite, sur le même tunnel{filters.tunnel === 'all' ? ' ou sur l’un des deux' : ''}, lus le {formatDate(report.coverage.optin.observedAt, true)}{report.coverage.optin.available ? ` ; hors calcul : ${formatNumber(report.coverage.optin.visitorsUnlinkable)} visiteur${(report.coverage.optin.visitorsUnlinkable ?? 0) > 1 ? 's' : ''} sans identifiant, ${formatNumber(report.coverage.optin.registrationsWithoutVisitor + report.coverage.optin.registrationsOutsideCohort)} inscription${report.coverage.optin.registrationsWithoutVisitor + report.coverage.optin.registrationsOutsideCohort > 1 ? 's' : ''} sans visite raccordable dans la période` : ` ; ${report.coverage.optin.reason ?? 'indisponible'}`}. Réservation = inscrits ayant un rendez-vous ÷ inscrits ; Présence = présents ÷ (présents + absents) sur les rendez-vous passés. Un tiret signifie qu’il manque une mesure, pas un zéro.</p><p className="a-d-data-note">Dernières lectures : inscriptions {formatDate(report.coverage.leads.observedAt, true)} · dépenses Meta jusqu’au {formatDate(report.coverage.meta.lastDay)} · ventes {formatDate(report.coverage.commerce.observedAt, true)} · visiteurs {report.coverage.visits.available ? `lus le ${formatDate(report.coverage.visits.observedAt, true)}` : 'non lus'}. Première publicité mesurée sur {formatNumber(report.totals.attributionBasis.firstTouch)} inscription{report.totals.attributionBasis.firstTouch > 1 ? 's' : ''}, arrivée de l’inscription sur {formatNumber(report.totals.attributionBasis.arrival)}.</p></>}
      </>}
    </div></div></section>;
}
...
export default function ResultsPage({ data, filters,posthog={},retryPosthog }: { data: DashboardResponse; filters: DashboardFilters;posthog?:PostHogClientState;retryPosthog?:()=>void }) {
  ...
  const selectedAd = /^meta-ad:(\d+)$/.exec(filters.campaign)?.[1];
  const currentCampaignId = /^meta:(\d+)$/.exec(filters.campaign)?.[1] ?? (selectedAd ? currentCampaignForAd(selectedAd) : null);
  const currentCampaign = CURRENT_MASTERCLASS_CAMPAIGNS.find(campaign => campaign.id === currentCampaignId);
  const campaignSelected = !!currentCampaign && !selectedAd && filters.campaign === `meta:${currentCampaign.id}`;
  return <div className="blg-results">
    {!campaignSelected && !selectedAd && <section className="a-d-stage results-kpis" aria-label="Indicateurs principaux"><header className="kpi-funnel-heading"><div><span className="blg-eyebrow">ENSEMBLE DE L’ACTIVITÉ</span><h2>Synthèse</h2><p>Du {formatDate(filters.from)} au {formatDate(filters.to)}</p></div></header><div className="a-d-cards">{data.metrics.map(item => <Card key={item.id} metric={item} compare={filters.compare} onOpen={setMetric} booked={data.pillars.flatMap(pillar => pillar.metrics).find(item => item.id === 'booked')} />)}</div></section>}
    {campaignSelected && <KpiFunnelTable filters={filters} revision={Date.parse(data.generatedAt) || 0} />}
    {currentCampaign && <AdFunnelTable filters={filters} />}
    {!campaignSelected && !selectedAd && <KpiFunnelTable filters={filters} revision={Date.parse(data.generatedAt) || 0} />}
    {!currentCampaign && <Chart series={data.series} />}
    {!currentCampaign && <><div className="results-section-heading"><h2>Comprendre les résultats</h2><span>Le détail par activité</span></div><div className="a-h-stage results-pillars">{data.pillars.map((pillar, i) => <Accordion key={pillar.id} title={pillar.title} marker={`0${i + 1}`} count={`${pillar.metrics.length} indicateurs`}><div className="results-pillar-metrics">{pillar.metrics.map(item => <button className="results-metric-row" key={item.id} onClick={() => setMetric(item)}><span>{name(item)}</span><strong>{formatNumber(item.value,item.unit)}</strong><span className="results-state">{item.id==='arrivals'&&posthog.current?.state==='waiting'&&posthog.current.retryable?'En attente':item.id==='arrivals' && posthog.current && ['loading','waiting'].includes(posthog.current.state)?'Chargement…':item.value === null ? 'Indisponible' : ''}</span><Arrow /></button>)}</div></Accordion>)}</div><div className="a-h-stage results-pillars"><Campaigns data={data} filters={filters} /><AdFunnelTable filters={filters} /></div></>}
    {metric && <MetricDrawer metric={metric} filters={filters} posthog={posthog} retryPosthog={retryPosthog} onClose={() => setMetric(null)} />}
  </div>;
}
