...
test('migration PostgreSQL17 complète, 24 tables RLS (dont le bail du tick, 017), droits client refusés tables/vues/RPC',async()=>{
 const rows=(await db.query("SELECT relname,relrowsecurity FROM pg_class WHERE relnamespace='public'::regnamespace AND relkind='r'")).rows;assert.equal(rows.length,24);assert.ok(rows.some(r=>r.relname==='cockpit_tick_lease'));assert.ok(rows.every(r=>r.relrowsecurity));
