...
try {
  ...
  await page.getByLabel('Du',{exact:true}).fill('2026-09-01'); await page.getByLabel('Au',{exact:true}).fill('2026-09-07'); await page.getByRole('button',{name:'Appliquer',exact:true}).click(); await ready(); pass('invalid dates rejected without applying the draft');
  ...
  await page.route('**/api/dashboard?*',async route=>{const state=structuredClone(dashboard);state.metrics=state.metrics.map((m,i)=>({...m,value:i===0?0:i===1?200:i===2?8:m.value,previous:i===0?100:i===1?100:i===2?0:null}));await route.fulfill({json:state});});
