...
try{
 ...
 assert.equal(await panel.locator('.kpi-funnel-scroll tbody tr').count(),response.snapshot.daily.length+3,'jours + 3 récapitulatifs');assert.deepEqual(await panel.locator('.kpi-funnel-scroll thead tr').last().locator('th').allInnerTexts(),['Dépense','Impressions','CPM','Clics lien','CTR','CPC','CTRU','Inscrits','Inscrits / clics','Coût par inscrit','Vues du CTA','Vues CTA / inscrits','Coût par vue du CTA','Formulaires','Formulaires / vues CTA','Coût par formulaire','Appels réservés','Réservés / vues du CTA','Coût par réservation','Inscrits → réservés','Appels planifiés','Appels réalisés','Présence','Offres faites','Offres / appels réalisés','Coût par offre','Ventes','Ventes / offres','Coût par client','Cash encaissé','CA contracté','ROAS cash','ROAS']);assert.match(await panel.locator('.kpi-funnel-scroll tbody tr').first().innerText(),/Global/);assert.match(await panel.innerText(),/12,34/);assert.match(await panel.innerText(),/Non mesuré/);assert.match(await panel.innerText(),/Lecture la plus ancienne/);
