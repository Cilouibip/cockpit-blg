...
`tests/refresh-mechanism.test.ts` (nouveau) : simulation du planificateur réel, profils central et pessimiste, déclenchement 2 min et 1 min, Notion delta 5 unités + relecture complète 36 unités par 24 h, scénarios « relecture la nuit » ; rapports JSON dans les preuves. `src/lib/sync-jobs.ts` `chooseSyncJob` : cadence la plus courte d'abord, garde de famine (retard de plus d'une cadence sur l'échéance), puis « touché le plus ancien » ; sans réglage (60 partout), inchangé. `tests/refresh-cycle.test.ts` : profil Notion post-021 (5 unités) et attente mesurée des flux horaires.

Migration `023_notion_full_window.sql` : `cockpit_claim_notion(text,text,jsonb)` (corps 021 + `p_schema.fullHours` facultatif : relecture complète 24 h après la précédente seulement dans la fenêtre Europe/Paris, et de toute façon dès 36 h ; forme invalide refusée 23514), bloc de retour arrière (corps 021). `src/lib/sync-notion-business.ts` : `notionFullHours` (réglage `BLG_NOTION_FULL_HOURS` « de-à », invalide = `notion_full_hours_invalid`, jamais de valeur par défaut) transmis avec la preuve de schéma ; `.env.example`. Tests : `tests/notion-full-window.test.ts` (nouveau), scénario 023 dans `tests/c3-notion-incremental.integration.ts` (le retour arrière 021 du scénario précédent est réappliqué).

`docs/ACTUALISATION.md` : §1.5 (mécanisme mesuré, tableau avant/après, limites), §2 (conditions : 017 à 023, `BLG_NOTION_FULL_HOURS`), §4 (`reappeared`, réapparition, croissance par jour), §5.1 (contrôle `BETWEEN 17 AND 23`), §6 (critères : valeurs simulées, seuil = décision de Mehdi), §7.1 (ordre protégé, requête « reprise nécessaire » prouvée dans `tests/state-rollback.integration.ts`). Aucun appel réel, aucun secret, aucune donnée nominative.

## 23 septembre 2026 : U9-notion-30, inventaire Notion tournant borné (migration 021, local, non appliqué)

Migration 021 : `cockpit_claim_notion(text,text,jsonb)` et `cockpit_publish_notion` remplacés (013 intacte). Delta = modifications `last_edited_time` + tranche d'inventaire : partitions d'environ 100 fiches découpées d'après `createdAt` du miroir (bornes au milieu des écarts), fusion des petites, `inventoriedAt` par partition, budget plafond(S × temps écoulé / 6 h) borné à [1/12, 1], plus anciennes d'abord (rotation `cursor` à égalité), retard de plus de 6 h relu d'office, partition nouvelle toujours lue. Publication : archivage limité aux plages relues avec une marge d'une minute (fiche sans `createdAt` : full seulement), `inventoryThrough` = plus ancienne `inventoriedAt`, partitions incohérentes = `DELTA_INVENTORY_GAP`. Bloc de retour arrière (corps 013, passage à tranche en cours mis en échec). `sync-jobs.ts` : `notion` flux pilote. `sync-notion-business.ts` : `coverage.inventoryPlan`.

Validation : `npm test` 602 → 605 ; `npm run test:db` 121 → 128 sur PostgreSQL 17 jetable ; contre-épreuves : restriction de l'archivage retirée, marge retirée, tests rouges. Commit `e3bfa4e` (branche `fable/u9-notion-30`), fusion `a71aa15` ; corrections de fusion par Fable : `state-tick-lease.test.ts` et `u8-freshness.test.ts` (Notion flux pilote), `docs/ACTUALISATION.md` §5.1 (021) et §5.2 (quatre conditions). Aucun appel réel, aucun secret, aucune donnée nominative.

## 23 septembre 2026 : tableau quotidien conforme à l'Excel et CTRU lu à la source (U8b, préparation locale)

Mission U8b déléguée par Fable, branche locale `fable/u8b-tableau` depuis `8f79728`, commits `57d3952` (étape A), `b5d2900` (étape B), `afa56dd`, `f337125`, `84f1acd` (échec isolé des lectures d'uniques Meta), fusion `9adb8f1`. Contrat `kpi-funnel-contract.ts` au schéma 3.0.0 : comptes `registrants`, `calls_booked`, `outbound_clicks`, `meta_reach`, `meta_unique_link_clicks`, taux nullables avec motif, couverture par bloc, récapitulatifs, garde-fous (taux = ratio exact de ses termes, taux inter-blocs seulement si chaque bloc couvre, inscrits jamais additionnés). `kpi-funnel-live.ts` : inscrits distincts par jour et fenêtre, appels réservés par `appointmentBooking`, récapitulatifs, CTRU. `kpi-funnel-export.ts` : colonnes de l'Excel partagées par la vue et le CSV. `kpi-meta.ts` : lecture campagne × jour inchangée, lecture compte × jour filtrée sur les campagnes Masterclass et six fenêtres (7 lectures de plus par passage) en domaine d'échec séparé ; `kpi-source-store.ts` : lignes `kpi_window_row`/`kpi_window_manifest` publiées par `cockpit_publish_aggregate_state` (018), lecteur `readKpiWindows` ; aucune migration. `.env.example` : `BLG_KPI_META_UNIQUE_READS`. Tests : 602 → 621 unitaires, 121 → 122 PostgreSQL (non-accumulation des fenêtres 6 → 6). Captures locales sans serveur ni accès (`.local/ux/kpi-u8b-*.png`). Aucune opération distante ; lecture Meta de contrôle, revue et fusion restent au coordinateur et à Codex.
