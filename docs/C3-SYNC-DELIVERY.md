...
- Ce lot initial conservait le cron minute17, la concurrence et la limite15min. Le coordinateur a ensuite publié PR16 avec trois réveils aux minutes17,37,57 ; ce clone ne modifie pas ce réglage. Le workflow draine dans une fenêtre12min et ne s'arrête plus au premier résultat d'unité en échec. Une erreur persistante termine toujours le workflow en échec.
...
Le coordinateur a intégré les lots et la migration013 ; le cycle manuel du 18 septembre à17:26 UTC a maintenant publié un vrai delta suivi de son inventaire. Il reste à observer plusieurs cycles automatiques et une fraîcheur≤1h. Les lecteurs conservent les fiches inchangées ; leur propre `sync_run_id` peut légitimement différer du dernier cycle. Aucun résultat local ni départ manuel ne valide la cadence réelle.
...
## Complément équité Commerce / Meta — 18 septembre 2026

Lecture du miroir à 16:42–16:43 UTC : le statut Meta en erreur correspondait encore à la tentative `ad_daily` de 14:41 (HTTP400, aucune ligne lue). Aucune nouvelle tentative Meta Ads n'avait encore commencé. Commerce avançait pourtant de 4 077 à 5 577 lignes par checkpoints de 27–36 s : le tri du scheduler utilisait uniquement son ancienne publication de 14:40, ignorant le stream `commerce_reader_checkpoint`. Chaque nouvelle invocation le replaçait devant Meta. Le succès global de 16:45:39 rapporté par le coordinateur confirme une attente avant Meta, pas un nouvel échec établi pendant ce diagnostic.

Correctif borné : une lecture supplémentaire du dernier checkpoint Commerce, métadonnées seulement et dans le même espace/profil, informe le rang du prochain travail, le verrou actif et le délai de reprise après échec. Après un chunk Commerce, les autres flux dus peuvent passer avant sa prochaine reprise. La dernière publication, `lastSuccessAt`, `dataAsOf` et la fraîcheur utilisent toujours exclusivement `commerce_declared_snapshot` ; aucun checkpoint ne devient un rapport disponible. Les limites de temps et de chunks restent inchangées.

Vérifications : contre-exemple de production reproduit en données synthétiques ; deux ticks au budget consommé servent Commerce puis Meta sans attendre une nouvelle publication commerciale ; verrou actif/expiré, checkpoint partiel/en échec, délai de reprise, dernière publication et isolation de profil/espace contrôlés. Aucun appel Meta, import parallèle, changement source, migration ou publication dans ce lot. Le HTTP400 historique ne conservait pas le sous-code Graph : sa cause précise n'est pas reconstruite par supposition.

## Complément découpage des checkpoints — 18 septembre 2026

Le run manuel GitHub35374311798 a réussi en24ticks, terminé à17:37:58 UTC. Notion a publié5modifications et contrôlé10250fiches par inventaire en106pages. Ses36unités totalisent95,190s de travail, dont73,894s source et20,614s base, étalées sur11min39s. Commerce totalise493,292s sur22unités, dont76,564s source et75,895s base. L'entrelacement explique l'attente de Notion ; il n'était pas bloqué. Les12flux sont complets et récents en sortie. La marge avant la fin du drainage reste faible. Aucun nouveau départ automatique n'est observé depuis14:31:48 UTC à l'heure de ce diagnostic ; le succès manuel ne clôt pas C3.

Défaut local isolé : `checkpointParts` sérialisait deux fois le fragment croissant pour chaque caractère. Il compte maintenant uniquement le coût JSON du caractère ajouté, enveloppe comprise. Les frontières persistées, le contenu, les empreintes, les limites2800octets/10000fragments et la préservation des paires UTF-16 restent identiques. Aucun changement de source, publication, migration, cadence, mesure métier ou budget.

Preuves : comparaison exacte avec l'ancien algorithme pour contrôles ASCII, échappements, accents, Unicode, surrogates isolés, chaînes pseudo-aléatoires et limites ; fixture existante de plusieurs mégaoctets et reprise après erreur. Mesure synthétique locale Node22 ARM64,3essais sur1,66Mo : avant6,80–8,70s, après0,123–0,136s,622fragments strictement identiques. Ce résultat ne promet pas une durée de production ; les340,833s Commerce hors appels réseau/base comprennent aussi d'autres traitements et ne sont pas intégralement attribuées à cette fonction. Les preuves privées sont `2026-09-18_notion-delta-workflow-measures.json`, `2026-09-18_notion-delta-live-diagnostic.json` et `2026-09-18_commerce-checkpoint-benchmark.json` dans le registre BLG.
