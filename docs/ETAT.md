...
- Notion : chaque passage lit les modifications et seulement une tranche de l'inventaire (1/12 à cadence 30, 1/6 à 60 ; tout l'inventaire relu en 6 h, garanti). Un passage courant tient en un appel du tick au lieu de neuf ; charge sur l'API Notion divisée par 4 environ. Lecture complète toutes les 24 h inchangée.
- Rendez-vous : flux pilote (60 minutes par défaut, 30 sur activation). Âge maximal après activation, déclenchement toutes les 2 minutes : 34 à 42 minutes en régime courant ; 52 minutes à 1 h 20 une fois par jour, pendant la relecture complète. Avant : jusqu'à 1 h 50.
- Contrepartie : une fiche supprimée dans Notion n'est retirée qu'au passage qui relit sa partition (6 h au plus ; 24 h si sa date de création est inconnue ou à moins d'une minute d'un bord). La fraîcheur de cette détection est publiée (`inventoryThrough`).

Preuves : 605 tests (deux tests qui supposaient Notion horaire adaptés à la fusion), typage et compilation ; 128 tests PostgreSQL 17 jetable (121 avant), dont sept scénarios U9 et deux contre-épreuves. Ordre : migration 021 appliquée avant l'activation de 30 (docs/ACTUALISATION.md §2, quatrième condition). Proposition non codée : relecture complète placée la nuit (décision Fable/Codex).

## 23 septembre 2026 : tableau « Suivi quotidien du funnel » aligné sur l'Excel du média buyer (U8b, local, revue requise)

Le tableau reprend les colonnes de l'Excel de référence, dans son ordre (dépense, impressions, CPM, clics lien, CTR, CPC, CTRU, inscrits et leurs taux, vues du CTA, formulaires, appels réservés, coût par réservation, inscrits → réservés, appels planifiés et réalisés, présence, offres, ventes, coût par client, cash, CA contracté, ROAS), en huit groupes, avec trois récapitulatifs datés au-dessus des jours (période, 3 et 7 derniers jours). Taux et coûts ont leur colonne ; un taux qui croise deux sources n'est calculé que si chacune couvre le jour ou toute la fenêtre, sinon « Non mesuré » avec le motif. Les récapitulatifs additionnent les jours puis calculent les taux ; ils ne moyennent jamais un taux. Inscrits = contacts distincts (proposition en attente de confirmation). Appels réservés = réservations des inscrits de la période, datées au jour de réservation (règle de date de la carte Résultats ; le périmètre cohorte contre carte Résultats attend l'arbitrage de Mehdi). Le coût par réservation divise la dépense du même jour (l'Excel prenait la veille). Le CTR unique est lu chez Meta par jour et par fenêtre (3, 7 et 30 jours), jamais additionné ni moyenné. Les formulaires restent grisés « sans objet », vues du CTA, offres et CA contracté non mesurés avec leur responsable. L'export suit l'ordre et les intitulés de l'Excel.

Validation locale : 621 tests, 122 tests PostgreSQL, compilation ; contre-épreuves faites (CTRU moyenné, coût sur la veille, taux sans couverture, isolation des lectures d'uniques). Limites : les nouvelles lectures Meta (niveau compte, fenêtres) n'ont jamais été exécutées réellement ; leur échec n'empêche plus la publication de la dépense (CTRU alors « non mesuré » avec le code) ; une lecture de contrôle par Codex reste requise pour mesurer le CTRU (interrupteur `BLG_KPI_META_UNIQUE_READS=off` en repli) ; onze ambiguïtés (correspondance et réalisation) attendent Mehdi.
