...
1. Migrations 017, 018, 019, 020, 021, 022 et 023 appliquées dans cet ordre, AVANT le déploiement du code de ce lot, hors passage (le code appelle `cockpit_publish_aggregate_state`, `cockpit_publish_meta_daily`, `cockpit_cleanup_staged` et lit `is_current` ; sans 019, le tick signale seulement `cleanup.error` = `schema_missing` ; 022 et 023 ne changent que des fonctions). Contrôle : `SELECT version, applied_at FROM cockpit_migrations WHERE version BETWEEN 17 AND 23 ORDER BY version;` renvoie sept lignes (17, 18, 19, 20, 21, 22, 23). Relevé 6.6 enregistré juste avant et juste après la migration 018 (la reprise ne supprime rien). Durées et verrous mesurés sur volume synthétique représentatif (673 000 lignes) : `private/derived/fable-cockpit-20260923/cp2-reprise-20260924/volume/RESULTATS.md`. **018 est en deux transactions** (reprise CP2) : DDL, fonctions et droits (verrous exclusifs de l'`ALTER TABLE` et des `CREATE INDEX`, moins d'une seconde), puis reprise des données qui inscrit la version (environ 75 s sur ce volume ; elle ne bloque pas les lectures, seulement les écritures des mêmes lignes : appliquer hors passage). Telle que livrée dans `748c6d0`, 018 tenait ces verrous exclusifs pendant toute la reprise : lectures et écritures du cockpit bloquées 74 s (sondes du relevé). Une reprise interrompue laisse le DDL en place sans version inscrite : rejouer 018 en entier (idempotent). 017, 019, 020, 021, 022, 023 : 25 à 35 ms chacune. `cockpit_resume_current_state()` (§7.1) : environ 60 s sur ce volume, même sans rien à reprendre (son parcours des périodes coûte autant qu'une reprise) : ne pas l'appeler une seconde fois pour vérifier, utiliser la requête « reprise nécessaire » de §7.1.
